...
class PruneMediaCommandTest extends TestCase
{
    ...
    public function test_it_deletes_media_without_files()
    {
        ...
        $media1 = factory(Media::class)->create(['_id' => 1, 'disk' => 'tmp']);
        $media2 = factory(Media::class)->create(['_id' => 2, 'disk' => 'tmp']);
        $this->seedFileForMedia($media2);

        $artisan->call('media:prune', ['disk' => 'tmp']);

        $this->assertEquals([2], Media::pluck('_id')->toArray());
    ...
    {
        $artisan = $this->getArtisan();
        $media1 = factory(Media::class)->create(['_id' => 1, 'disk' => 'tmp', 'directory' => '']);
        $media2 = factory(Media::class)->create(['_id' => 2, 'disk' => 'tmp', 'directory' => 'foo']);

        $artisan->call('media:prune', ['disk' => 'tmp', '--directory' => 'foo']);

        $this->assertEquals([1], Media::pluck('_id')->toArray());
        $this->assertEquals("Pruned 1 record(s).\n", $artisan->output());
    }
